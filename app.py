...
# 1. تعريف الجدول بشكل مبدئي بـ None لمنع خطأ NameError تماماً
df = None
...
try:
    ...
    # في حال حدوث خطأ، يظهر تنبيه احترافي دون انهيار الموقع
    st.error(f"❌ السيرفر غير قادر على قراءة ملف الإكسل حالياً.")
    st.info("💡 السبب: السيرفر لم يقم بتثبيت أداة openpyxl بعد. يرجى مراجعة الخطوة رقم 2 بالأسفل.")

# 2. حماية الموقع: لا يتم تشغيل القوائم إلا إذا تم تحميل الجدول بنجاح
if df is not None:
    ...
    st.warning("⚠️ النظام في انتظار تفعيل جدار الحماية والأدوات من السيرفر الرئيسي...")
